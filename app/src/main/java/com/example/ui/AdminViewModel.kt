...
class AdminViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val _staffDetailState = MutableStateFlow<UiState<StaffAdmin>>(UiState.Idle)
    val staffDetailState: StateFlow<UiState<StaffAdmin>> = _staffDetailState

    /** Result of an approve/reject/disable/enable/delete/edit action - separate from
     * staffState/staffDetailState so the UI can show a one-off success/error message
     * (e.g. a snackbar) without disturbing whatever list/detail is currently shown. */
    private val _staffActionState = MutableStateFlow<UiState<String>>(UiState.Idle)
    val staffActionState: StateFlow<UiState<String>> = _staffActionState

    private val _staffRegisterState = MutableStateFlow<UiState<String>>(UiState.Idle)
    val staffRegisterState: StateFlow<UiState<String>> = _staffRegisterState
    ...
    fun logout() {
        viewModelScope.launch {
            ...
            _staffDetailState.value = UiState.Idle
            _staffActionState.value = UiState.Idle
     ...
     * When [autoAnalyze] is false (the "AI Analyze" toggle is OFF), the photo is attached to
     * the draft as usual but no AI call is made - the admin fills in every field manually,
     * and the product can still be saved normally.
     */
    fun onImageSelected(uri: String, bitmap: Bitmap, autoAnalyze: Boolean = true) {
    ...
    // ================= STAFF MANAGEMENT (Head Admin only on the backend) =================
    fun loadStaff() {
        viewModelScope.launch {
    ...
    fun loadStaffDetail(id: Int) {
        viewModelScope.launch {
            _staffDetailState.value = UiState.Loading
            try {
                val res = getApi().getStaffDetail(id)
                if (res.success && res.data != null) _staffDetailState.value = UiState.Success(res.data)
                else _staffDetailState.value = UiState.Error(res.message)
            } catch (e: Exception) {
                _staffDetailState.value = UiState.Error(friendlyError(e))
            }
        }
    }

    fun clearStaffDetail() {
        _staffDetailState.value = UiState.Idle
    }

    fun resetStaffActionState() {
        _staffActionState.value = UiState.Idle
    }

    /** action is one of "approve", "reject", "disable", "enable". */
    fun performStaffAction(id: Int, action: String) {
        viewModelScope.launch {
            _staffActionState.value = UiState.Loading
            try {
                val res = getApi().staffAction(StaffActionRequest(id, action))
                if (res.success) {
                    _staffActionState.value = UiState.Success(res.message)
                    loadStaff()
                } else {
                    _staffActionState.value = UiState.Error(res.message)
                }
            } catch (e: Exception) {
                _staffActionState.value = UiState.Error(friendlyError(e))
            }
        }
    }

    fun updateStaffProfile(id: Int, edit: StaffEditRequest) {
        viewModelScope.launch {
            _staffActionState.value = UiState.Loading
            try {
                val res = getApi().updateStaff(id, edit)
                if (res.success) {
                    _staffActionState.value = UiState.Success(res.message)
                    if (res.data != null) _staffDetailState.value = UiState.Success(res.data)
                    loadStaff()
                } else {
                    _staffActionState.value = UiState.Error(res.message)
                }
    ...
    fun resetStaffRegisterState() {
        _staffRegisterState.value = UiState.Idle
    }

    /** Staff self-registration, called from the pre-login "Register as Staff" screen. */
    fun registerStaff(
        name: String,
        age: String,
        gender: String,
        ...
        if (name.isBlank() || username.isBlank() || phone.isBlank() || password.isBlank()) {
            _staffRegisterState.value = UiState.Error("Name, username, phone and password are required")
            return
        }
        viewModelScope.launch {
            _staffRegisterState.value = UiState.Loading
            try {
                ...
                val api = ApiClient.getBackendApi(apiBaseUrl.first(), "")
                val res = api.registerStaff(
                    name = text(name.trim()),
                    age = text(age.trim()),
                    gender = text(gender.trim()),
                    username = text(username.trim()),
                    phone = text(phone.trim()),
                    password = text(password),
                    geminiApiKey = text(geminiApiKey.trim()),
                    photo = photoPart
                )
                if (res.success) {
                    _staffRegisterState.value = UiState.Success(res.message)
                } else {
                    _staffRegisterState.value = UiState.Error(res.message)
                }
            } catch (e: Exception) {
                _staffRegisterState.value = UiState.Error(friendlyError(e))
            }
        }
    }
