...
// A staff member's own Gemini key (set at registration or edited by the
// Head Admin) takes priority over the shared shop-wide key in Settings,
// so each staff member's usage is billed to their own key when they have
// one. Falls back to the shared key otherwise - this keeps the existing
// Head Admin / single-key setup working exactly as before.
$apiKey = '';
$stmt = $conn->prepare("SELECT gemini_api_key FROM admins WHERE id = ?");
$stmt->execute([$adminId]);
$row = $stmt->fetch();
if ($row && !empty($row['gemini_api_key'])) {
    $personalKey = decryptSecretValue($row['gemini_api_key'], $CONFIG);
    if ($personalKey) {
        $apiKey = $personalKey;
    }
}
...
if (!$apiKey) {
    sendJson(false, "AI is not configured yet. Go to Settings -> AI and save your API key, or add your own Gemini key to your staff profile.", null, 400);
}
