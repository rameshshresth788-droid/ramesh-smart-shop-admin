...
// Returns the admin id, or sends a 401/403 JSON response and exits if not authorized.
// Also re-checks approval_status on every call (not just at login), so a staff
// account that gets disabled/rejected mid-session immediately loses access
...
function verifyAdmin($conn) {
    ...
    $stmt = $conn->prepare("SELECT id, token_expires_at, approval_status FROM admins WHERE token = ?");
    $stmt->execute([$token]);
    $admin = $stmt->fetch();
    if (!$admin) {
    ...
    if (array_key_exists('approval_status', $admin) && $admin['approval_status'] !== null && $admin['approval_status'] !== 'APPROVED') {
        sendJson(false, "Your account is no longer approved. Please contact the Head Admin.", null, 403);
    }
    return $admin['id'];
}
...
// True/false version of requireHeadAdmin's role check, without sending a
// response - useful when an endpoint needs to branch behavior by role
// rather than hard-reject non-head-admins outright.
function isHeadAdmin($conn, $adminId) {
    $admin = getAdminById($conn, $adminId);
    return $admin && $admin['role'] === 'HEAD_ADMIN' && $admin['approval_status'] === 'APPROVED';
}
